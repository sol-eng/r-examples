...
main_df <- read_parquet("data/all_year_sales.parquet") %>%
...
sales_total_week_city <- main_df %>%
  group_by(week = floor_date(`Order Date`, 'weeks'), City) %>%
  summarize(Sales = sum(total_sales, na.rm = T)) %>%
  ungroup()
...
sales_total_week_state <- main_df %>%
  group_by(week = floor_date(`Order Date`, 'weeks'), State) %>%
  summarize(Sales = sum(total_sales, na.rm = T)) %>%
  ungroup()
...
sales_total_week_category <- main_df %>%
  group_by(week = floor_date(`Order Date`, 'weeks'), Category) %>%
  summarize(Sales = sum(total_sales, na.rm = T)) %>%
  ungroup()
